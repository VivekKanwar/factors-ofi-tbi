...
label_A <- glue("Trauma registry 2012–2023\n{_n_total_merged_data} patients")
label_B <- glue("Identified TBI using AIS 1xxxxx.x\n{n_tbi_TBI_only_data} patients with TBI")
label_C <- glue("Excluded:\n<18 years: {n_under18}\nDead on arrival: {n_doa}")
label_D <- glue("Linked with trauma care quality database\nExcluded: no OFI outcome: {n_no_ofi}")
label_E <- glue("Complete-case selection\nExcluded: missing data in covariates: {n_missing_cc}")
label_F <- glue("Final analytic cohort\n{n_final} adult TBI patients\nOFI: {n_ofi} ({round(100*n_ofi/n_final,1)}%) | No OFI: {n_final - n_ofi}")
